// This file serves as the backend script for the add review web page. It facilitates the display of the three separate forms used for different review types as well as 
// the reminder choice display and provides functionality for the five star rating elements, and finally validates form submission for each of the three forms.

// First we get the buttons on the landing screen for the review page and add event listeners for them to display the correct forms depending on the button clicked. 
...
// This function simply sets all children of the main div container to hidden and reveals the Day Form, changing the header text in the process.
...
// This function simply sets all children of the main div container to hidden and reveals the Reminder tiles, changing the header text in the process.
...
// This function simply sets all children of the main div container to hidden and reveals the Other Form, changing the header text in the process.
...
// It then assigns event listeners for the click events of the cancel buttons on each form.
...
// Each of these event listeners displays the previous page and clear all fields on the current form.
...
// This function simply sets all children of the main div container to hidden and reveals the initial landing page, changing the header text in the process.
...
// This function clears all fields in the Day Review form.
...
// This function clears all fields in the Reminder Review form.
...
// This function clears all fields in the Other Review form.
...
// Next, it adds an event listener for the back button on the Reminder Choice form for a function which displays the initial landing page.
...
// It then adds an event listener for each reminder tile in the Reminder Choice form.
// The event listener calls the displayReminderReviewForm function to progress through the review-making process.
...
// This function simply sets all children of the main div container to hidden and reveals the Reminder Review form, changing the header text in the process.
// Additionally, it set finds the current reminder chosen and assigns it to currentReminder so it can be used to populate the Reminder Review form. 
function displayReminderReviewForm()
{
    ...
    currentReminder = elapsedReminders.find(r => r.reminderID == reminderID);
    newReviewHeader.innerText = "New Review for " + currentReminder.name;
    // If the reminder is one time, the datePicker does not need to be displayed.
...
// This function is used to reset the star rating for the passed in form, so viewing the form again will show no rating selected.
function resetStars(form)
{
    let starContainer = null;
    // It first determines which stars should be selected by the passed form.
    if (form == "day")
    {
    ...
    // It then removes each star and adds five new ones, adding the appropriate event listener in the process.
    for (let counter = 0; counter < 5; counter++)
    {
